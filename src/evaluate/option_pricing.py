...
class OptionPricingEngine:
    """Class for option pricing engine."""
    def __init__(
    ...
    def exact_european_prices(self, K):
        """Calculate exact European option prices."""
    ...
    def exact_lookback_prices(self, T_cur):
        """Calculate exact lookback option prices."""
    ...
    def exact_asian_prices(self, K):
        """Calculate exact Asian option prices."""
        M1 = (np.exp(self.r * self.T) - 1) / (self.r * self.T) * self.S0
        M2 = (
            2 * np.exp((2 * self.r + self.sigma**2) * self.T) * self.S0**2 / 
            ((self.r + self.sigma**2) * (2 * self.r + self.sigma**2) * self.T**2) + 
            2 * self.S0**2 / (self.r * self.T**2) * 
            (1 / (2 * self.r + self.sigma**2) - np.exp(self.r * self.T) / (self.r + self.sigma**2))
        )
    ...
    def calc_all_T(self, grid_size=1, approx_df=None, recalculate_all=False, resample=True, recalculate_input=False):
        """Calculate all option prices for different maturities."""
        self.calculate_option_prices_T(
            grid_size=grid_size, approx_df=approx_df, recalculate_all=recalculate_all, 
            resample=resample, recalculate_input=recalculate_input
        )
    ...
    def calc_all_K(self, K_values=np.linspace(0.5, 1.5, 100), approx_df=None, recalculate_all=False, resample=True, recalculate_input=False):
        """Calculate all option prices for different strike prices."""
        self.calculate_option_prices_K(
            K_values=K_values, approx_df=approx_df, recalculate_all=recalculate_all, 
            resample=resample, recalculate_input=recalculate_input
        )
        self.calculate_option_price_deviation_absolute()
        self.calculate_option_price_deviation_relative()
        ...
        for i, T_cur in enumerate(self.ground_paths_df.columns):
            ...
            if resample:
                ...
                mc_call_gen_price = np.mean(
                    np.exp(-self.r * (T_cur - self.t)) * self.call_payoff(sample_df.iloc[:, :(i + 1)])
                )
                mc_put_gen_price = np.mean(
                    np.exp(-self.r * (T_cur - self.t)) * self.put_payoff(sample_df.iloc[:, :(i + 1)])
                )
            else:
                mc_call_gen_price = np.mean(
                    np.exp(-self.r * (T_cur - self.t)) * self.call_payoff(self.gen_paths_df.iloc[:, :(i + 1)])
                )
                mc_put_gen_price = np.mean(
                    np.exp(-self.r * (T_cur - self.t)) * self.put_payoff(self.gen_paths_df.iloc[:, :(i + 1)])
                )
            ...
            if recalculate or recalculate_input:
                if recalculate:
                    if self.approx_exact:
                        exact_call_price = np.mean(
                            np.exp(-self.r * (self.T - self.t)) * self.call_payoff(approx_df.iloc[:, :(i + 1)])
                        )
                        exact_put_price = np.mean(
                            np.exp(-self.r * (self.T - self.t)) * self.put_payoff(approx_df.iloc[:, :(i + 1)])
                        )
                    else: 
                ...
                mc_call_ground_price = np.mean(
                    np.exp(-self.r * (T_cur - self.t)) * self.call_payoff(self.ground_paths_df.iloc[:, :(i + 1)])
                )
                mc_put_ground_price = np.mean(
                    np.exp(-self.r * (T_cur - self.t)) * self.put_payoff(self.ground_paths_df.iloc[:, :(i + 1)])
                )
    ...
    def calculate_option_prices_K(self, K_values, approx_df=None, recalculate_all=False, resample=True, recalculate_input=False):
        ...
        for K in K_values:
            if resample:
                ...
                mc_put_gen_price = np.mean(
                    np.exp(-self.r * (self.T - self.t)) * self.put_payoff(sample_df, K)
                )
            else:
                mc_call_gen_price = np.mean(
                    np.exp(-self.r * (self.T - self.t)) * self.call_payoff(self.gen_paths_df, K)
                )
                mc_put_gen_price = np.mean(
                    np.exp(-self.r * (self.T - self.t)) * self.put_payoff(self.gen_paths_df, K)
                )
            ...
            if recalculate or recalculate_input:
                if recalculate:
                    if self.approx_exact:
                        exact_call_price = np.mean(
                            np.exp(-self.r * (self.T - self.t)) * self.call_payoff(approx_df, K)
                        )
                        exact_put_price = np.mean(
                            np.exp(-self.r * (self.T - self.t)) * self.put_payoff(approx_df, K)
                        )
                    else: 
                ...
                mc_call_ground_price = np.mean(
                    np.exp(-self.r * (self.T - self.t)) * self.call_payoff(self.ground_paths_df, K)
                )
                mc_put_ground_price = np.mean(
                    np.exp(-self.r * (self.T - self.t)) * self.put_payoff(self.ground_paths_df, K)
                )
    ...
    def calculate_option_price_deviation_absolute(self):
        """Calculate absolute deviations of option prices."""
        if not self.input_is_real_data:
    ...
    def calculate_option_price_deviation_relative(self):
        """Calculate relative deviations of option prices."""
        if not self.input_is_real_data:
